...
//                .securitySchemes(Arrays.asList(apiKey()))
                ...
                .select()
                .apis(
                        // 컨트롤러 패키지 설정 => 이 안에있는 컨트롤러만 찾는다
                        RequestHandlerSelectors.basePackage("com.teosprint.flashcard.controller")
                                // 그 중에서도 RestController Annotation 만을 찾아서 설정
                                .and(RequestHandlerSelectors.withClassAnnotation(RestController.class))) // controller가 있는 패키지 지정 basePackage
                .paths(PathSelectors.any())
                .build()
                .apiInfo(apiInfo());
    }
    ...
    private ApiInfo apiInfo() {
        return new ApiInfoBuilder()
                .title("Teo sprint")
                .description("2022-08-27 api docs!")
                .version("0.1")
                .build();
    }
